...
import { getCajeros, toggleCajeroEstado } from 'services/cajeroService'; 
...
const ListarCajeros = () => {
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            render: (row) => (
                <button 
                    onClick={() => setCajeroToToggle({ id: row.id, estado: row.estado, nombre: row.datos_empleado?.nombre })}
    ...
    const fetchCajeros = useCallback(async (page, search = '', status = '') => {
        setLoading(true);
        try {
            const response = await getCajeros(page, search, status);
            const { content, currentPage, totalPages } = response.data;
            setCajeros(content);
            setPaginationInfo({ currentPage, totalPages });
        } catch (err) {
            setAlert({ type: 'error', message: 'Error al cargar los cajeros.' });
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    useEffect(() => {
        const handler = setTimeout(() => {
            fetchCajeros(1, searchTerm, statusFilter);
        }, 500);
        return () => clearTimeout(handler);
    }, [searchTerm, statusFilter, fetchCajeros]);

    const executeToggleEstado = async () => {
        if (!cajeroToToggle) return;
        const { id } = cajeroToToggle;
        setCajeroToToggle(null);
        try {
            await toggleCajeroEstado(id);
            setAlert({ type: 'success', message: 'Estado del cajero actualizado correctamente.' });
            fetchCajeros(paginationInfo.currentPage, searchTerm, statusFilter);
        } catch (err) {
    ...
    return (
        <div className="container mx-auto p-6 min-h-screen">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-serif font-bold text-restaurant-primary">Gestión de Cajeros</h1>
                    <p className="text-sm text-gray-500 mt-1">Administra el personal encargado de la caja en tu sede.</p>
                </div>
                
                <Link 
                    to="/admin/agregar-cajero" 
            ...
            {cajeroToToggle && (
                <ConfirmModal
                    message={`¿Estás seguro de cambiar el estado de ${cajeroToToggle.nombre}? Si lo desactivas, no podrá acceder al sistema.`}
            ...
            <Table 
                ...
                pagination={{
                    ...
                    onPageChange: (page) => fetchCajeros(page, searchTerm, statusFilter)
                }}
            />
        </div>
    );
