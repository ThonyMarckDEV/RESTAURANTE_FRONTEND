...
import { showCaja, updateCaja } from 'services/cajaService'; 
...
const EditarCaja = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await showCaja(id);
        const { nombre } = response.data;
        setFormData({ nombre: nombre || '' });
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    setAlert(null);
    try {
      const response = await updateCaja(id, formData);
      setAlert({ type: 'success', message: response.message || 'Caja actualizada correctamente.' });
      ...
      <div className="flex justify-between items-center mb-8 border-b-2 border-restaurant-secondary/20 pb-4">
          <div className="flex items-center gap-3">
             ...
             <div>
                 ...
                 <p className="text-sm text-gray-500">Actualizar nombre del punto de venta</p>
             </div>
          </div>
          <button 
